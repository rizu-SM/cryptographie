...
def des_encrypt(text, key):
    text_bin = str_to_bin(text).ljust(64,'0')[:64]
    key_bin = str_to_bin(key).ljust(64,'0')[:64]

    keys = generate_keys(key_bin)

    block = permute(text_bin, IP)
    ...
    return hex(int(final,2))[2:]

# -------------------- DECRYPT --------------------

def des_decrypt(cipher_hex, key):
    bin_data = format(int(cipher_hex,16),'064b')
    key_bin = str_to_bin(key).ljust(64,'0')[:64]

    keys = generate_keys(key_bin)[::-1]

    block = permute(bin_data, IP)
    left, right = block[:32], block[32:]
